...
class KidsProductListScreen extends StatefulWidget {
  const KidsProductListScreen({Key? key}) : super(key: key);

  @override
  State<KidsProductListScreen> createState() => _KidsProductListScreenState();
}

class _KidsProductListScreenState extends State<KidsProductListScreen> {
  int _selectedFilter = 0;
  final List<String> filters = ["All", "Boys", "Girls", "Newborn", "Sale"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("KIDS' COLLECTION", style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16, color: theme.colorScheme.primary)),
        ...
        children: [
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text("SUMMER 2024", style: GoogleFonts.poppins(fontSize: 11, color: Colors.white60, letterSpacing: 2)),
              const SizedBox(height: 8),
              Text("THE MINI MINIMALIST", style: GoogleFonts.poppins(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 8),
              Text("Playful styles crafted for little ones.", style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey[300])),
            ]),
          ),
          Expanded(
            child: StreamBuilder<List<Product>>(
              stream: _productRepository.watchProducts(category: 'Kids'),
