...
class MenProductListScreen extends StatefulWidget {
  const MenProductListScreen({Key? key}) : super(key: key);

  @override
  State<MenProductListScreen> createState() => _MenProductListScreenState();
}

class _MenProductListScreenState extends State<MenProductListScreen> {
  int _selectedFilter = 0;
  final List<String> filters = ["All", "New", "Suits", "Outerwear", "Casual"];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("MEN'S FASHION", style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16, color: theme.colorScheme.primary)),
        ...
        children: [
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text("SELECTED CURATIONS", style: GoogleFonts.poppins(fontSize: 11, color: Colors.white60, letterSpacing: 2)),
              const SizedBox(height: 8),
              Text("THE MODERN ATELIER", style: GoogleFonts.poppins(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 8),
              Text("A curated selection of timeless pieces.", style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey[300])),
            ]),
          ),
          Expanded(
            child: StreamBuilder<List<Product>>(
              stream: _productRepository.watchProducts(category: 'Men'),
