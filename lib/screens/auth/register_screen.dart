...
class _RegisterScreenState extends State<RegisterScreen> {
          ...
          SizedBox(
            ...
            child: Stack(children: [
              ...
              Positioned(bottom: 24, left: 24, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Create\nAccount', style: GoogleFonts.poppins(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.white)),
                const SizedBox(height: 4),
                Text('Join LUXE today', style: GoogleFonts.poppins(fontSize: 14, color: Colors.white.withOpacity(0.7))),
              ])),
            ]),
          ),
          Form(
              ...
              child: Column(children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : Text("Register Now", style: GoogleFonts.poppins(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                  ),
                ),
                const SizedBox(height: 16),
                _buildDivider(theme),
  ...
  Widget _buildInput({required TextEditingController controller, required String label, required IconData icon, bool isPassword = false, bool isConfirm = false, required ThemeData theme}) {
    return TextFormField(
      controller: controller, obscureText: isPassword && (isConfirm ? _obscureConfirm : _obscurePassword),
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: isPassword ? IconButton(icon: Icon((isConfirm ? _obscureConfirm : _obscurePassword) ? Icons.visibility_off : Icons.visibility, color: theme.colorScheme.onSurface.withOpacity(0.4)), onPressed: () => setState(() { if(isConfirm) _obscureConfirm = !_obscureConfirm; else _obscurePassword = !_obscurePassword; })) : null,
      ),
    );
  }
  ...
  Widget _buildLoginRow(ThemeData theme) {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      Text("Already have an account? ", style: GoogleFonts.poppins(color: theme.colorScheme.onSurface.withOpacity(0.6), fontSize: 14)),
      TextButton(onPressed: () => Navigator.pushNamed(context, AppRoutes.login), child: Text("Login", style: GoogleFonts.poppins(color: AppColors.secondaryColor, fontWeight: FontWeight.w600))),
    ]);
  }
  ...
  Future<void> _handleRegister() async {
    ...
    try {
      await repo.registerWithEmailPassword(email: email, password: password.trim());
